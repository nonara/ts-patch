...
import { addDiagnosticFactory } from './patcher';
...
function createTransformerFromPattern({ typescript, factory, config, program, ls }: {
...
export class PluginCreator {
  ...
  createTransformers(
    ...
    // Chain custom tranformers at the end
  ...
  private resolveFactory(transform: string, importKey: string = 'default'): PluginFactory | undefined {
    ...
    if (!factory)
      throw new Error(
        `tsconfig.json > plugins: "${transform}" does not have an export "${importKey}": ${inspect(factoryModule)}`
      );
